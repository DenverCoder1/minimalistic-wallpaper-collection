...
const REPO = "DenverCoder1/minimalistic-wallpaper-collection";
const BRANCH_NAME = "main";
const IMAGES_DIRECTORY = "images";
const BASE_URL = `https://raw.githubusercontent.com/${REPO}/${BRANCH_NAME}/${IMAGES_DIRECTORY}/`;
const IMGPROXY_PREFIX = `https://dc1imgproxy.fly.dev/x/rs:auto:332:200:1/plain/${encodeURIComponent(
  BASE_URL
)}`;
const GITHUB_API_URL = `https://api.github.com/repos/${REPO}/contents/${IMAGES_DIRECTORY}/`;
const DOWNLOAD_ZIP_URL = `https://github.com/${REPO}/archive/refs/tags/${VERSION}.zip`;

// Fetch the contents of a URL
async function fetchContents(url: string, userAgent: string): Promise<string> {
...
async function fetchBinary(url: string, userAgent: string): Promise<ArrayBuffer> {
  const response = await fetch(url, {
    headers: {
      "User-Agent": userAgent,
    },
...
app.get("/images/:filename", async (c) => {
  ...
  const redirect = c.req.query("redirect") === "1" || true;
  ...
  const contents = await fetchBinary(imagePath, REPO);
  
  // Redirect if the image is larger than 4.5MB
  if (contents.byteLength > 4500000) {
    return c.redirect(imagePath);
  }
...
app.get("/random", async (c) => {
  ...
  const imagesResponse = await fetch(GITHUB_API_URL, {
    headers: {
      "User-Agent": REPO,
    },
  });
  ...
  if (!Array.isArray(images) || images.length === 0) {
    return c.text("Error: images could not be parsed from GitHub API", 500);
  }
  
  // Pick a random image
  const randomImage = images[Math.floor(Math.random() * images.length)];
  
  if (!randomImage || !randomImage.download_url) {
    return c.text("Error: image was not in expected format", 500);
  }
  
  const randomImagePath = randomImage.download_url;
  
  if (redirect) {
    return c.redirect(randomImagePath);
  }
  
  const contents = await fetchBinary(randomImagePath, REPO);
...
<body>
    ...
    <div class="icons">
        ...
        <!-- Download -->
        <a href="${DOWNLOAD_ZIP_URL}" target="_blank" title="Download">
    ...
    <div class="gallery">
        ${images.map((image: any) => {
          const imagePath = image.download_url;
          const filename = imagePath.split("/").pop();
          return `
            <a href="${imagePath}" class="glightbox" data-alt="${filename}" data-description="${filename}">
                <img src="${IMGPROXY_PREFIX}${filename}" loading="lazy" alt="${filename}" title="${filename}" class="loading" onload="this.classList.remove('loading')">
            </a>
          `;
        }).join("")}
    </div>
    ...
    <div class="footer">
        ...
        <p>Website by <a href="https://github.com/DenverCoder1/">Jonah Lawrence</a>, &copy; ${new Date().getFullYear()}</p>
    </div>
